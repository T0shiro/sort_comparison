...
def run_sorts(List<Closure> closures, iterations, max_pow) {
    Random rand = new Random()
    List<Map<Integer, Double>> meansList = new ArrayList<>()
    List<List<Integer>> resultsList = new ArrayList<>()
    for (int i = 0; i < closures.size(); i++) {
        meansList.add(new HashMap<>())
        resultsList.add([])
    }
    for (int i = 1; i <= max_pow; i++) {
        def sz = 2**i
...
quicksort_threshold = [new quick_sort(algorithms.quick_sort.Pivot.RANDOM, 2**7).&run,
                       new quick_sort(algorithms.quick_sort.Pivot.RANDOM, 2**6).&run,
                       new quick_sort(algorithms.quick_sort.Pivot.RANDOM, 2**5).&run,
                       new quick_sort(algorithms.quick_sort.Pivot.RANDOM, 2**4).&run,
                       new quick_sort(algorithms.quick_sort.Pivot.RANDOM, 2**3).&run]

quicksort_threshold_names = ["threshold-2^7.txt",
                       "threshold-2^6.txt",
                       "threshold-2^5.txt",
                       "threshold-2^4.txt",
                       "threshold-2^3.txt"]
...
create_output_files(quicksort_threshold, quicksort_threshold_names)
